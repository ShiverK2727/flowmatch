...
def setup_dataloaders(config):
    ...
    train_dataset = CelebaHQDataset(os.path.join(data_config['base_dir'], 'train'), transform=train_transform)
    val_dataset = CelebaHQDataset(os.path.join(data_config['base_dir'], 'val'), transform=val_transform)
...
def setup_trainer(config, wandb_logger, exp_dir, gpu_ids, train_dataset_size):
    ...
    log_info(f"  - Effective batch size (with {num_gpus} GPUs): {effective_batch_size}", print_message=True)
...
def main(args):
    ...
    if wandb_logger:
        ...
        wandb_logger.log_hyperparams({"args": vars(args)})

...
def setup_model(config, output_dir):
    """设置模型"""
...
def setup_trainer(config, wandb_logger, exp_dir, gpu_ids, train_dataset_size):
    ...
    if 'every_n_train_steps' in trainer_config:
        ...
        log_info(f"  - Save every {trainer_config['checkpoint_every_n_epochs']} epochs")
...
def main(args):
    ...
    # 设置模型
    model = setup_model(config, exp_dir)
    ...
    wandb_logger = WandbLogger(
        name=logger_name,
        project=config['logging'].get('project_name', 'cifar_dit'),
        save_dir=exp_dir,
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='DiT Training Script for CIFAR-10')
